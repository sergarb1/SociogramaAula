...
## Build
```bash
npm run build:css      # Regenerate css/tailwind.css from Tailwind classes
```

**css/tailwind.css** is committed to git (needed for GitHub Pages). After editing Tailwind classes in HTML, run `npm run build:css` and commit the updated file.
...
├── css/tailwind.css    # Compiled Tailwind (auto-generated by `npm run build:css`)
├── js/
