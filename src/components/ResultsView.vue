<template>
  ...
  <template v-if="mode === 'results'">
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-6">
      ...
      <div class="lg:col-span-1 space-y-4">
        ...
        <button @click="$emit('organize')" class="w-full py-2.5 rounded-xl text-sm font-bold transition shadow-sm bg-green-600 text-white hover:bg-green-700 flex items-center justify-center gap-1.5">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z"/></svg>
          {{ t('teams.form') }}
        </button>
        <button @click="$emit('dist')" class="w-full py-2.5 rounded-xl text-sm font-bold transition shadow-sm bg-indigo-600 text-white hover:bg-indigo-700 flex items-center justify-center gap-1.5">
  ...
  <template v-if="mode === 'teams'">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-5">
      <div class="flex items-center justify-between mb-4">
        ...
        <button @click="$emit('back')" class="text-xs text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 transition flex items-center gap-1">
      ...
      <div class="flex flex-wrap items-center gap-3 mb-4">
        <label class="text-xs text-slate-500 dark:text-slate-400">{{ t('teams.strategy') }}:</label>
        <select v-model="teamStrategy" class="px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-xs text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-300">
          <option v-for="s in teamStrategies" :key="s.id" :value="s.id">{{ t(s.labelKey) }}</option>
        </select>
        ...
        <button v-if="teams.length" @click="exportTeamsPNG" class="px-3 py-1.5 rounded-xl text-xs font-medium bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-600 transition shadow-sm flex items-center gap-1.5">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.41a2.25 2.25 0 013.182 0l2.909 2.91m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"/></svg>
          {{ t('teams.exportPNG') }}
        </button>
      </div>
      <div v-if="teams.length" id="teamsContainer" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
  ...
  <template v-if="mode === 'dist'">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-5">
      ...
      <div class="flex flex-wrap items-center gap-3 mb-4">
        <label class="text-xs text-slate-500 dark:text-slate-400">{{ t('dist.strategy') }}:</label>
        <select v-model="distStrategy" class="px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-xs text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-300">
          <option v-for="s in distStrategies" :key="s.id" :value="s.id">{{ t(s.labelKey) }}</option>
        </select>
        <label class="text-xs text-slate-500 dark:text-slate-400">{{ t('dist.rows') }}:</label>
        <input v-model.number="distRows" type="number" min="1" max="10" class="w-14 px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-300">
        <label class="text-xs text-slate-500 dark:text-slate-400">{{ t('dist.columns') }}:</label>
        <input v-model.number="distCols" type="number" min="1" max="10" class="w-14 px-2 py-1.5 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-700 text-sm text-slate-800 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-indigo-300">
        <button @click="generateDist" class="px-4 py-1.5 rounded-xl text-sm font-medium bg-indigo-600 text-white hover:bg-indigo-700 transition shadow-sm">{{ t('dist.generate') }}</button>
        <button v-if="distGrid.tables.length > 0" @click="exportDistPNG" class="px-3 py-1.5 rounded-xl text-xs font-medium bg-white dark:bg-slate-700 border border-slate-200 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-600 transition shadow-sm flex items-center gap-1.5">
          <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M2.25 15.75l5.159-5.159a2.25 2.25 0 013.182 0l5.159 5.159m-1.5-1.5l1.409-1.41a2.25 2.25 0 013.182 0l2.909 2.91m-18 3.75h16.5a1.5 1.5 0 001.5-1.5V6a1.5 1.5 0 00-1.5-1.5H3.75A1.5 1.5 0 002.25 6v12a1.5 1.5 0 001.5 1.5zm10.5-11.25h.008v.008h-.008V8.25zm.375 0a.375.375 0 11-.75 0 .375.375 0 01.75 0z"/></svg>
          {{ t('dist.exportPNG') }}
        </button>
      </div>
      <div v-if="distGrid.tables.length > 0" id="distContainer" class="overflow-x-auto">
        <div class="inline-flex flex-col gap-2 mx-auto">
          <div v-for="(row, ri) in distGrid.tables" :key="ri" class="flex gap-2 justify-center">
            <div v-for="(cell, ci) in row" :key="ci"
              @dragover.prevent="distDragOver(ri, ci)"
              @drop.prevent="distDrop(ri, ci)"
              class="w-28 h-20 rounded-xl border-2 border-dashed flex flex-col items-center justify-center transition-all cursor-default select-none"
              :class="cell.length ? 'border-indigo-300 dark:border-indigo-600 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-600 bg-slate-50 dark:bg-slate-800/50'">
              <template v-if="cell.length">
                <span draggable="true" @dragstart="distDragStart(cell[0])"
                  class="w-7 h-7 rounded-full flex items-center justify-center text-white text-xs font-bold cursor-grab active:cursor-grabbing shrink-0 shadow-sm"
                  :style="'background:'+stringToColor(cell[0].name)"
                  :title="cell[0].name">{{ cell[0].name.charAt(0) }}</span>
                <span class="text-[10px] text-slate-600 dark:text-slate-300 mt-0.5 truncate max-w-24 text-center leading-tight">{{ cell[0].name }}</span>
              </template>
              <span v-else class="text-[10px] text-slate-300 dark:text-slate-600">{{ t('dist.empty') }}</span>
            </div>
          </div>
        </div>
        <p class="text-xs text-slate-400 dark:text-slate-500 text-center mt-4">{{ t('dist.dragHint') }}</p>
      </div>
      <div v-if="!distGrid.tables.length" class="text-center py-8 text-slate-400 dark:text-slate-500 text-xs">{{ t('dist.generate') }}</div>
    </div>
  </template>
</template>
...
import type { TeamStrategy, DistStrategy, DistGrid } from '@/utils/teams'
import { renderGraph, destroyGraph } from '@/utils/graph'
import { exportGraphPNG, exportElementPNG, downloadStudentsCSV, downloadMatrixCSV, downloadAnonymizedJSON, downloadAnonymizedReportHTML, exportReportHTML } from '@/utils/reports'
import { downloadJSON, saveDistribution, loadDistribution } from '@/utils/storage'
...
const teamStrategy = ref<TeamStrategy>('balanced')
const teamStrategies = getTeamStrategies()
const distStrategy = ref<DistStrategy>('balanced')
const distStrategies = getDistStrategies()
const distGrid = ref<DistGrid>({ rows: 4, cols: 4, tables: [] })
const distRows = ref(4)
const distCols = ref(4)
const distSeed = ref(Date.now())
...
function generateTeams() {
  const t = formTeams(props.group.students, matrix.value, roles.value, teamSize.value, Date.now(), teamStrategy.value)
  teams.value = t
}

function generateDist() {
  distSeed.value = Date.now()
  const g = formDistribution(props.group.students, matrix.value, roles.value, distRows.value, distCols.value, distSeed.value, distStrategy.value)
  distGrid.value = g
  saveDistribution(props.group.id, g.tables, g.rows, g.cols)
}

function exportTeamsPNG() {
  exportElementPNG('teamsContainer', `equipos-${props.group.name}-${new Date().toISOString().slice(0, 10)}.png`, props.lang)
}

function exportDistPNG() {
  exportElementPNG('distContainer', `distribucion-${props.group.name}-${new Date().toISOString().slice(0, 10)}.png`, props.lang)
}

let dragDistStudent: Student | null = null
function distDragStart(s: Student) {
  dragDistStudent = s
}
function distDragOver(r: number, c: number) {
  if (!dragDistStudent) return
}
function distDrop(r: number, c: number) {
  if (!dragDistStudent) return
  const table = distGrid.value.tables
  const oldR = table.findIndex(row => row.some(cell => cell.some(s => s?.id === dragDistStudent!.id)))
  let oldC = -1
  if (oldR >= 0) {
    oldC = table[oldR].findIndex(cell => cell.some(s => s?.id === dragDistStudent!.id))
  }
  const dragStud = dragDistStudent
  if (oldR >= 0 && oldC >= 0) {
    table[oldR][oldC] = table[oldR][oldC].filter(s => s.id !== dragStud!.id)
  }
  table[r][c] = [...table[r][c].filter(Boolean), dragStud]
  dragDistStudent = null
  distGrid.value = { ...distGrid.value, tables: table }
  saveDistribution(props.group.id, table, distRows.value, distCols.value)
}

function exportPNG() {
  destroyGraph()
  nextTick(() => {
    exportGraphPNG('resultsGraph', props.lang).then(() => renderIt())
  })
}
