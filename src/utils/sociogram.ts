import type { Student, Matrix, Metrics, Prediction, Question } from '@/constants'
...
export function computeSociogram(
  students: Student[],
  ...
  const choicesCount = Object.fromEntries(students.map(s => [s.id, 0])) as Record<string, number>
  const rejectionsCount = Object.fromEntries(students.map(s => [s.id, 0])) as Record<string, number>
...
export function computeFromMatrix(students: Student[], matrix: Matrix): SociogramResult {
  const choicesCount = Object.fromEntries(students.map(s => [s.id, 0])) as Record<string, number>
  const rejectionsCount = Object.fromEntries(students.map(s => [s.id, 0])) as Record<string, number>
