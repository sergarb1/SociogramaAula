...
export function renderGraph(
  ...
  if (!container || typeof Network === 'undefined') return null
  ...
  const edges: Array<{
    from: string; to: string; color: Record<string, string>; width: number;
    arrows?: { to: { enabled: boolean; scaleFactor: number } };
    dashes?: number[]; smooth?: { type: string; roundness: number };
    title?: string;
  }> = []
  ...
  if (onNodeClick) {
    networkInstance.on('click', (p: { nodes: string[] }) => {
