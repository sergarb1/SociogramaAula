import { generateId, loadGroups, saveGroups, saveResponses } from './storage'
import type { Student, Group } from '@/constants'
...
export function generateRandomStudents(count: number): Student[] {
...
function generateRealisticResponses(students: Student[], activeQuestions: Array<{ maxChoices: number; type: string }>): Record<string, Record<string, string[]>> {
...
export async function generateTestData(): Promise<Group[]> {
  const pool = BUILTIN_TEMPLATES.filter(t => t.students.length > 0)
  const pick = [pool[5], pool[3], pool[2]]
  const groups: Group[] = []
  const allResponses: Record<string, Record<string, Record<string, string[]>>> = {}
  ...
  for (const tmpl of pick) {
    ...
    const baseQs = [
      { text: '¿A quién elegirías para un trabajo?', type: 'Elección positiva', maxChoices: 3 },
      { text: '¿A quién NO querrías en tu equipo?', type: 'Rechazo', maxChoices: 2 },
      { text: '¿Quién es más popular?', type: 'Percepción positiva', maxChoices: 1 },
    ]
